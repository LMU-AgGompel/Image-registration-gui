...
def average_energy_along_curve(pts, energy_map):
    ...
    Parameters:
        pts (numpy.ndarray): A 2D array of shape (n, 2) representing a list of points.
                             The x-coordinates are in the first column and y-coordinates are in the second column.
    ...
    """
    energy_values = ndimage.interpolation.map_coordinates(energy_map, [pts[:, 1], pts[:, 0]], order=1)
...
def average_energy_along_curve_interp(flattened_pts, energy_map, memo=None):
    ...
    energy_values = ndimage.interpolation.map_coordinates(energy_map, [new_y, new_x], order=1)
    return np.mean(energy_values)
...
def crop_image_around_active_contour(image, pts, margin=0.10):
    ...
    xmin = int((1 - margin) * np.min(pts[:, 0]))
    xmax = int((1 + margin) * np.max(pts[:, 0]))
    ymin = int((1 - margin) * np.min(pts[:, 1]))
    ymax = int((1 + margin) * np.max(pts[:, 1]))
...
def position_starting_points_active_contour(pts_ref, p_start, p_end, flip_pts = False):
    ...
    Parameters:
        pts_ref (numpy.ndarray): A 2D array of shape (n, 2) representing a list of points.
                                 The x-coordinates are in the first column and y-coordinates are in the second column.
        p_start (tuple): A tuple of (p_start_x, p_start_y) representing the coordinates of the desired starting point.
        p_end (tuple): A tuple of (p_end_x, p_end_y) representing the coordinates of the desired ending point.
    ...
    # Extract the x and y coordinates from pts_ref
    x_ref, y_ref = pts_ref[:, 0], pts_ref[:, 1]

    # Translate the points to make the first point of pts_ref coincide with p_start
    pts_ref -= [x_ref[0], y_ref[0]]

    # Compute the scaling factor (uniform scaling on both axes)
    scaling_factor = np.hypot(p_end[1] - p_start[1], p_end[0] - p_start[0]) / np.hypot(y_ref[-1] - y_ref[0], x_ref[-1] - x_ref[0])
    scaling_factor = np.abs(scaling_factor)
    pts_ref = scaling_factor * pts_ref

    # Compute the rotation angle
    angle_start_end = np.arctan2(p_end[1] - p_start[1], p_end[0] - p_start[0])
    angle_ref = np.arctan2(pts_ref[-1, 1], pts_ref[-1, 0])
...
def fit_active_contour(energy, p_start, p_end, pts_relative_seed, flip_pts, 
    ...
    Parameters:
        ...
        p_start (tuple): Starting point coordinates (p_start_x, p_start_y).
        p_end (tuple): Ending point coordinates (p_end_x, p_end_y).
...
def fit_multiple_contours_model(image, landmarks_dict, landmarks_ref_dict, multiple_contours_model, plot=False):
    ...
    for idx, model in multiple_contours_model.iterrows():
        ...
        pts_relative_seed = np.array([pts_relative_seed_x, pts_relative_seed_y]).T
...
def position_starting_points_active_contour(pts_ref, p_start, p_end, flip_pts = False):
    ...
    # Translate the points so that the first point is in the origin
    pts_translated = np.array([x_ref, y_ref]).T-np.array([x_ref[0], y_ref[0]])

    # Compute the scaling factor (uniform scaling on both axes)
    scaling_factor = np.hypot(p_end[1] - p_start[1], p_end[0] - p_start[0]) / np.hypot(y_ref[-1] - y_ref[0], x_ref[-1] - x_ref[0])
    scaling_factor = np.abs(scaling_factor)
    pts_translated = scaling_factor*pts_translated

    # Compute the rotation angle
    start_end_vector = [p_end[0] - p_start[0], p_end[1] - p_start[1]]
    
    angle_start_end = np.arctan2(start_end_vector[1], start_end_vector[0])
    angle_ref = np.arctan2(pts_translated[-1, 1], pts_translated[-1, 0])
    rotation_angle = angle_start_end - angle_ref
    ...
    # Apply the affine transformation to pts_ref
    transformed_pts_ref = np.dot(pts_translated, rotation_matrix) + np.array(p_start)
...
def fit_active_contour(energy, p_start, p_end, pts_relative_seed, flip_pts, 
    ...
    # 1. Define the active contour initial positions:
    pts = position_starting_points_active_contour(pts_relative_seed, p_start, p_end, flip_pts)
    displacements = pts[1:] - pts[0:-1]
    point_distances = np.sqrt(displacements[:, 0]**2 + displacements[:, 1]**2)
    cumul_dist = np.cumsum(point_distances)
    cumul_dist = np.insert(cumul_dist, 0, 0)
...
def fit_multiple_contours_model(image, landmarks_dict, landmarks_ref_dict, multiple_contours_model, plot=False):
    ...
    for idx, model in multiple_contours_model.iterrows():
        ...
        if plot:
            # Plot fitted contours and equispaced points using distinct colors
            plt.scatter(contour_pts[:, 0], contour_pts[:, 1], c=colors[idx], s=5)
            plt.scatter(equispaced_pts[:, 0], equispaced_pts[:, 1], c=colors[idx], s=30)     
